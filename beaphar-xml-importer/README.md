...
A WordPress/WooCommerce plugin that automatically imports and updates Beaphar-branded products from an XML feed provided by Miazoo.bg. The plugin synchronizes product data, manages inventory, and handles pricing calculations daily.
...
- Automatically assigns the "Beaphar" brand taxonomy
...
- Located at: **Tools → Beaphar Import**
...
- Filters products by brand: "Beaphar"
...
2. **Filter Products**
   - Extracts only products with brand = "Beaphar"
...
3. **Process Products in Batches**
   ...
   - Syncs the following data:
     ...
     - **Stock quantity**
     - **Brand taxonomy** (Beaphar)

4. **Handle Missing Products**
   - Identifies Beaphar products not in the current feed
   - Sets them to out of stock (quantity: 0)

### Product Matching
The plugin identifies Beaphar products by:
- Brand taxonomy: `pa_brand = "beaphar"`
- SKU starting with: `BEA`

## Installation

1. Upload `beaphar-xml-importer.php` to `/wp-content/plugins/` directory
...
### Manual Import
1. Go to **WordPress Admin → Tools → Beaphar Import**
...
- `add_action('beaphar_daily_stock_update')` - Runs import
